...
function ratingClick(ev) {
    ...
    var score = this.getAttribute('data-score');
    var paper_id = this.getAttribute('data-paper-id');
    var feedback = $('#id_feedback_'+paper_id).val();
    rate(url, score, paper_id, feedback, this);
}
 ...
 * Rate instance
 *********************/
function rate(url, score, paper_id, feedback, sender) {
    rest.post(url, {'score': score}, function (rating) {
		$.ajax({
            url: "/editors/update-rating/?paper_id="+paper_id,
            type: "POST",
 ...
 *********************/
// Call Init function when file is included
init();
